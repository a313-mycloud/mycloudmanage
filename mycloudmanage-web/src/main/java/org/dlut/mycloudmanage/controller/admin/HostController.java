...
public class HostController extends BaseController {
    ...
    public String adminHostList(HttpServletRequest request, HttpServletResponse response, ModelMap model,
                                Integer currentPage) {
        ...
        Pagination<HostDTO> pageHostDTO = this.hostBiz.query(queryHostCondition);
        if (pageHostDTO.getTotalPage() >= 1 && (currentPage < 1 || currentPage > pageHostDTO.getTotalPage()))
    ...
    public String addHostForm(HttpServletRequest request, HttpServletResponse response, ModelMap model) {
        String errorDesc = this.setDefaultEnv(request, response, model);
        if (errorDesc != null) {
            return this.goErrorPage(errorDesc);
        }
    ...
    public String editForm(HttpServletRequest request, HttpServletResponse response, ModelMap model, int hostId) {
        String errorDesc = this.setDefaultEnv(request, response, model);
        if (errorDesc != null) {
            return this.goErrorPage(errorDesc);
        }
        ...
        HostDTO hostDTO = this.hostBiz.query(queryHostCondition).getList().get(0);
    ...
    @ResponseBody
    public String editHost(HttpServletRequest request, HttpServletResponse response, ModelMap model, int hostId,
        ...
        if (page2.getTotalCount() > 0 && !page1.getList().get(0).getHostIp().equals(hostIp)) {
            json.put("isSuccess", false);
